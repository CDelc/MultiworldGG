    ...
    def char_needs_tags(state: CollectionState,tag_list,jump_height):
        for i in character_info:
            if state.has(i,player):
                if subsetWorkaround(character_info,tag_list):
                    return True
        return False

    def subsetWorkaround(biglist, smalllist):
        for i in smalllist:
            if i not in biglist:
                return False
        return True
...
#115
        ...
        add_rule(world.get_location("Deep Sea (Act 2) Star Emblem", player),
                 lambda state: (state.has("Yellow Springs",player) or state.has("Gargoyle Statues",player)) and char_needs_tags(state, ['strong_floors'], -1))#technically this requires either jump height or 'strong_walls' but no reasonable character would make this impossible
        ...
        add_rule(world.get_location("Castle Eggman (Act 2) Diamond Emblem", player),
                 lambda state: (state.has("Swinging Maces", player) and state.has("Yellow Springs", player) and char_needs_tags(state, ["low_grav"], -1)) or
                               ...
                               char_needs_tags(state, ['climbs_walls',"low_grav"], -1) or char_needs_tags(state, [], 1500))
...
#spin path requires zoom tubes, ys, 350jump or climb or wj
        ...
        add_rule(world.get_location("Pipe Towers Heart Emblem", player),
                 lambda state: char_needs_tags(state, ["climbs_walls"], 200) or char_needs_tags(state, ['wall_jump'], -1) or
