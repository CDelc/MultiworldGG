...
class PhantomHourglassClient(DSZeldaClient):
    ...
    async def update_special_key_count(self, ctx, current_stage: int, new_keys, key_data: dict, key_values) -> list[tuple[int, list, str]]:
        res = []
        if current_stage == 37:
        ...
        write_list = [(key_data["address"], [new_keys], "Main RAM")]
        if key_data["name"] != "Temple of the Ocean King":
            reset_tracker = (~key_data["filter"]) & key_values["tracker"]
            res.append((key_data["address"], [reset_tracker], "Main RAM"))
        return res
    ...
    async def process_game_completion(self, ctx: "BizHawkClientContext"):
        current_scene = self.read_result["scene"]
        game_clear = False
